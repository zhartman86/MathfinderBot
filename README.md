...
## Commands
There are several other commands than the ones listed below. However, I've only documented the ones I feel are worth using at the moment. There are also `dm` versions of some commands, which add an additional `target` option for selecting other player's active sheets.
